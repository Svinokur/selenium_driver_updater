...
class testEdgeDriver(unittest.TestCase):
    ...
    def test01_check_get_current_version_edge_selenium_failure(self):
        current_version = self.edgedriver_failure._EdgeDriver__get_current_version_edgedriver()
        self.assertEqual(len(current_version), 0, len(current_version))

    #@unittest.skip('Temporary not needed')
    def test02_check_download_driver(self):
        try:
            file_name = self.edgedriver_failure._EdgeDriver__download_driver(version=self.specific_version_failure)
    ...
    def test03_check_compare_current_version_and_latest_version_failure(self):
        try:
            is_driver_is_up_to_date, current_version, latest_version = self.edgedriver_failure._EdgeDriver__compare_current_version_and_latest_version()
    ...
    def test05_check_if_version_is_valid_failure(self):
        url = 'blablablanoturl'
        try:
            self.edgedriver_failure._EdgeDriver__check_if_version_is_valid(url=url)
        except Exception as e:
            self.assertTrue(e.__class__ == DriverVersionInvalidException, e.__class__)
    ...
    def test07_check_get_current_version_edge_selenium(self):
        current_version = self.edgedriver._EdgeDriver__get_current_version_edgedriver()
    ...
    def test08_check_download_driver_specific_version(self):
        self.edgedriver._EdgeDriver__delete_current_edgedriver_for_current_os()
        self.assertFalse(Path(self.edgedriver_path).exists(), self.edgedriver_path)

        file_name = self.edgedriver._EdgeDriver__download_driver(version=self.specific_version)
        ...
        self.edgedriver._EdgeDriver__chmod_driver()
    ...
    def test09_check_download_driver_latest_previous_version(self):
        ...
        file_name = self.edgedriver._EdgeDriver__download_driver(previous_version=True)
        self.assertIsNotNone(file_name,file_name)
        self.assertGreater(len(file_name), 0, len(file_name))
        self.assertTrue(Path(self.edgedriver_path).exists(), self.edgedriver_path)
    ...
    def test10_check_get_latest_version_edge_driver(self):
        latest_version = self.edgedriver._EdgeDriver__get_latest_version_edgedriver()
    ...
    def test12_check_download_driver(self):
        ...
        file_name = self.edgedriver._EdgeDriver__download_driver()
        self.assertIsNotNone(file_name,file_name)
        self.assertGreater(len(file_name), 0, len(file_name))
        self.assertTrue(Path(self.edgedriver_path).exists(), self.edgedriver_path)
    ...
    def test13_check_compare_current_version_and_latest_version(self):
        is_driver_is_up_to_date, current_version, latest_version = self.edgedriver._EdgeDriver__compare_current_version_and_latest_version()
    ...
    def test16_check_if_version_is_valid(self):
        ...
        self.edgedriver._EdgeDriver__check_if_version_is_valid(url=url)
