...
class testGeckoDriver(unittest.TestCase): 
    ...
    def test01_check_get_current_version_geckodriver_selenium_failure(self):
        try:
            current_version = self.gecko_driver_failure._GeckoDriver__get_current_version_geckodriver()
    ...
    def test02_check_download_driver(self):
        try:
            file_name = self.gecko_driver_failure._GeckoDriver__download_driver(version=self.specific_version_failure)
    ...
    def test03_check_compare_current_version_and_latest_version_failure(self):
        try:
            is_driver_is_up_to_date, current_version, latest_version = self.gecko_driver_failure._GeckoDriver__compare_current_version_and_latest_version()
    ...
    def test05_check_if_version_is_valid_failure(self):
        url = 'blablablanoturl'
        try:
            self.gecko_driver_failure._GeckoDriver__check_if_version_is_valid(url=url)
        except Exception as e:
            self.assertTrue(e.__class__ == DriverVersionInvalidException, e.__class__)
    ...
    def test07_check_download_driver_specific_version(self):
        ...
        file_name = self.gecko_driver._GeckoDriver__download_driver(version=self.specific_version)
    ...
    def test08_check_download_driver_latest_previous_version(self):
        ...
        file_name = self.gecko_driver._GeckoDriver__download_driver(previous_version=True)
        self.assertIsNotNone(file_name,file_name)
        self.assertGreater(len(file_name), 0, len(file_name))
        self.assertTrue(Path(self.geckodriver_path).exists(), self.geckodriver_path)
