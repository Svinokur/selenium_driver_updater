...
class testChromeDriver(unittest.TestCase):
    ...
    def test01_check_get_current_version_chrome_selenium_failure(self):
        current_version = self.chrome_driver_failure._ChromeDriver__get_current_version_chromedriver()
    ...
    def test03_check_compare_current_version_and_latest_version_failure(self):
        is_driver_is_up_to_date, current_version, latest_version = self.chrome_driver_failure._ChromeDriver__compare_current_version_and_latest_version()
    ...
    def test07_check_get_result_by_request(self):
        latest_version = self.chrome_driver._ChromeDriver__get_latest_version_chromedriver()
    ...
    def test08_check_get_current_version_chrome_selenium(self):
        current_version = self.chrome_driver._ChromeDriver__get_current_version_chromedriver()
    ...
    def test09_check_download_driver_specific_version(self):
        self.chrome_driver._ChromeDriver__delete_current_chromedriver_for_current_os()
        self.assertFalse(Path(self.chromedriver_path).exists(), self.chromedriver_path)

        file_name = self.chrome_driver._ChromeDriver__download_driver(version=self.specific_version)
        ...
        self.chrome_driver._ChromeDriver__chmod_driver()
    ...
    def test10_check_download_driver_latest_previous_version(self):
        ...
        file_name = self.chrome_driver._ChromeDriver__download_driver(previous_version=True)
        self.assertIsNotNone(file_name,file_name)
        self.assertGreater(len(file_name), 0, len(file_name))
        self.assertTrue(Path(self.chromedriver_path).exists(), self.chromedriver_path)
    ...
    def test13_check_download_driver(self):
        ...
        file_name = self.chrome_driver._ChromeDriver__download_driver()
        self.assertIsNotNone(file_name,file_name)
        self.assertGreater(len(file_name), 0, len(file_name))
        self.assertTrue(Path(self.chromedriver_path).exists(), self.chromedriver_path)
    ...
    def test14_check_compare_current_version_and_latest_version(self):
        is_driver_is_up_to_date, current_version, latest_version = self.chrome_driver._ChromeDriver__compare_current_version_and_latest_version()
    ...
    def test17_check_if_version_is_valid(self):
        ...
        self.chrome_driver._ChromeDriver__check_if_version_is_valid(url=url)
