...
class testGithubViewer(unittest.TestCase): 
    ...
    #@unittest.skip('Temporary not needed')
    def test02_check_get_latest_release_data_by_repo_name(self):
        result, message, json_data = GithubViewer.get_specific_asset_by_repo_name(asset_name = self.specific_asset_name, repo_name=self.repo_name)
        self.assertTrue(result, message)
        self.assertIsNotNone(json_data,json_data)
        self.assertGreater(len(json_data), 0, len(json_data))

    #@unittest.skip('Temporary not needed')
    def test03_check_get_specific_asset_by_specific_version_by_repo_name(self):
        result, message, json_data = GithubViewer.get_specific_asset_by_specific_version_by_repo_name(version = self.specific_version, asset_name = self.specific_asset_name, repo_name=self.repo_name)
        self.assertTrue(result, message)
        self.assertIsNotNone(json_data,json_data)
        self.assertGreater(len(json_data), 0, len(json_data))
