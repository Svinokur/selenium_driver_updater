...
class Extractor():
    ...
    @staticmethod
    def extract_and_detect_archive_format(archive_path : str, out_path : str, delete_archive : bool = True) -> Tuple[bool, str]:
        ...
        try:
            ...
            if archive_path.endswith('.zip'):
                ...
                result, message = Extractor.extract_all_zip_archive(archive_path=archive_path, out_path=out_path, delete_archive=delete_archive)
                if not result:
                    logging.error(message)
                    return result, message
                ...
                result, message = Extractor.extract_all_tar_gz_archive(archive_path=archive_path, out_path=out_path, delete_archive=delete_archive)
                if not result:
                    logging.error(message)
                    return result, message
                ...
                result, message = Extractor.extract_all_tar_bz2_archive(archive_path=archive_path, out_path=out_path, delete_archive=delete_archive)
                if not result:
                    logging.error(message)
                    return result, message
